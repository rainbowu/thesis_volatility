...
# returns the task and the address space for the mapping dalvikof the data
...
def parseJavaLangString(stringObj):
    #print "---------------------------------------"
    #print "received object: %s" % getString(stringObj.clazz.descriptor)
    if getString(stringObj.clazz.descriptor)+"" != "Ljava/lang/String;":
        return "This is not a StringObject (%s)" % getString(stringObj.clazz.descriptor)

    #for k in stringObj.obj_vm:
    #    for offset, hex, chars in utils.Hexdump(stringObj.obj_vm[k]):
    ...
    #print "---> count(%d) - offset(%d) - value(%d)" % (count,offset,value)
    if offset == 0:
        if count < 1000:
            ###### Parsing ArrayObject ######
